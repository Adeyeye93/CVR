...
const Backup = () => {
  ...
  const initLocalDirectory = async (directoryHandle, prompt = true) => {
    ...
    if (permissions) {
      let videoTitle = `Chrome Video Recorder - ${new Date().toLocaleString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
        hour: "numeric",
      ...
      alert(
        "Failed to set up local backup. Reach out to us at crvhelp@meetingt.us for more help. You can still record your screen."
      );
  ...
  const directoryPicker = async (prompt = true) => {
    ...
    if (directoryPicker.name === "CRV Recordings") {
  ...
  const localSaving = async (prompt = true) => {
    ...
    if ("showDirectoryPicker" in window) {
      ...
      alert(
        "Your browser doesn't support local backups. Reach out to us at crvhelp@meetingt.us.io for more help. You can still record your screen."
      );
      chrome.storage.local.set({ backup: false });
      chrome.runtime.sendMessage({
        type: "backup-created",
